...
sphere2 = Sphere([2, 0, 4], 1, [0, 0, 255], 500, 0.3)
sphere3 = Sphere([-2, 0, 4], 1, [0, 255, 0], 10, 0.4)
...
def traceRay (origin, direction, t_min, t_max, recursive_depth):
	closest_sphere, closest_t = closestIntersection(origin, direction, t_min, t_max)

	if closest_sphere == None:
		return BACKGROUND_COLOR

	# calculation of local color
	point = np.add(O, np.dot(closest_t, direction))
	normal = np.subtract(point, closest_sphere.center)
	normal = normal / np.linalg.norm(normal)
	local_color = np.dot(closest_sphere.color, computeLighting(point, normal, np.multiply(-1, direction), closest_sphere.specular))

	# base case for recursion
	r = closest_sphere.reflective
	if recursive_depth <= 0 or r <= 0:
		return local_color

	# recursive case to calculate reflective color
	reflect_ray = reflectRay(np.multiply(-1, direction), normal)
	reflected_color = traceRay(point, reflect_ray, 0.001, math.inf, recursive_depth - 1)

	return np.add(np.multiply(1 - r, local_color), np.multiply(r, reflected_color))

def computeLighting (point, normal, direction, specular):
	total_intensity = 0.0
	...
	for light in lights:
		if light.typeL == Light.AMBIENT:
			...
			# shading
			shadow_sphere, shadow_t = closestIntersection(point, vec_light, 0.001, t_max)
			...
			# specular reflection
			if specular != -1:
				R = 2 * np.dot(normal, np.dot(normal, vec_light)) - vec_light
				r_dot_v  = np.dot(R, direction)
				if r_dot_v > 0:
					total_intensity += light.intensity * math.pow(r_dot_v/(np.linalg.norm(R) * np.linalg.norm(direction)), specular)
...
def reflectRay (reflect_ray, normal):
	return np.subtract(2 * np.dot(normal, np.dot(normal, reflect_ray)), reflect_ray)
...
def main ():
	...
	for i in range(CANVAS_WIDTH):
		for j in range(CANVAS_HEIGHT):
			...
			color = traceRay(O, direction, 1, math.inf, RECURSION_DEPTH)
